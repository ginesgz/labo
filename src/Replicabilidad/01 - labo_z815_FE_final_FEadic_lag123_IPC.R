...
AgregarVariables  <- function( dataset )
{
  ...
  dataset[ , fe_cdebitos_auto_total  := rowSums( cbind( ccuenta_debitos_automaticos,  ctarjeta_visa_debitos_automaticos, ctarjeta_master_debitos_automaticos) , na.rm=TRUE ) ] #no salio importante con canaritos 0.3
  dataset[ , fe_mdebitos_auto_total  := rowSums( cbind( mcuenta_debitos_automaticos,  mttarjeta_visa_debitos_automaticos, mttarjeta_master_debitos_automaticos) , na.rm=TRUE ) ] #no salio importante con canaritos 0.3
  
  dataset[ , fe_cdescuentos_total    := rowSums( cbind( ccajeros_propios_descuentos,  ctarjeta_visa_descuentos, ctarjeta_master_descuentos) , na.rm=TRUE ) ] #no salio importante con canaritos 0.3
  dataset[ , fe_mdescuentos_total    := rowSums( cbind( mcajeros_propios_descuentos,  mtarjeta_visa_descuentos, mtarjeta_master_descuentos) , na.rm=TRUE ) ] #no salio importante con canaritos 0.3
  
  dataset[ , fe_ccomisiones_total       := rowSums( cbind( ccomisiones_mantenimiento,  ccomisiones_otras) , na.rm=TRUE ) ] #no salio importante con canaritos 0.3 
  dataset[ , fe_mcomisiones_total       := rowSums( cbind( mcomisiones_mantenimiento,  mcomisiones_otras) , na.rm=TRUE ) ] #IMPORTANTE
  
  dataset[ , fe_ctransferencias_total   := rowSums( cbind( ctransferencias_recibidas,  ctransferencias_emitidas) , na.rm=TRUE ) ] #no salio importante con canaritos 0.3
  dataset[ , fe_mtransferencias_total   := rowSums( cbind( mtransferencias_recibidas,  mtransferencias_emitidas) , na.rm=TRUE ) ] #no salio importante con canaritos 0.3
  
  dataset[ , fe_mactivopasivo        := rowSums( cbind( mpasivos_margen,  mactivos_margen) , na.rm=TRUE ) ] #IMPORTANTE
  
  dataset[ , fe_mprestamos_total     := rowSums( cbind( mprestamos_personales,  mprestamos_prendarios, mprestamos_hipotecarios) , na.rm=TRUE ) ] #IMPORTANTE
  
  dataset[ , fe_mplazofijo_total     := rowSums( cbind( mplazo_fijo_dolares,  mplazo_fijo_pesos) , na.rm=TRUE ) ] #no salio importante con canaritos 0.3
  
  dataset[ , fe_inversiones_total    := rowSums( cbind( minversion1_pesos,  minversion1_dolares, minversion2) , na.rm=TRUE ) ] #no salio importante con canaritos 0.3
  
  dataset[ , fe_edad_por_antiguedad  := cliente_edad * cliente_antiguedad ] #no salio importante con canaritos 0.3
  dataset[ , fe_antiguedad_div_edad  := cliente_antiguedad / cliente_edad] #IMPORTANTE
  
  # normalizo la rentabilidad anual si la antiguedad del cliente es menor a 12 meses
  dataset[  , fe_mrentabilidad_annual_normalizada := mrentabilidad_annual ] 
  dataset[ cliente_antiguedad < 12 , fe_mrentabilidad_annual_normalizada := mrentabilidad_annual * 12 / cliente_antiguedad ] #IMPORTANTE 
...
#cargo el dataset AJUSTADO POR INFLACION A MARZO 2021
...
dir.create( "./exp/FE8150_FEadic_lag123_IPC/", showWarnings = FALSE )
setwd("./exp/FE8150_FEadic_lag123_IPC/")   #Establezco el Working Directory DEL EXPERIMENTO
...
#Mi edit: eliminio variables antes de agregar lags, deltas, tendencias, etc. 
...
#grabo el dataset
fwrite( dataset,
        "dataset_815_FEadic_lag123_IPC.csv.gz",
        logical01= TRUE,
        sep= "," )